...
export default async function CourseMaterialsPage({ params }: PageProps): Promise<ReactElement> {
  const { courseId } = await params;
  const supabase = await createClient();

  const { data: { user } } = await supabase.auth.getUser();
  if (!user) redirect(`/login?redirect=/dashboard/teacher/courses/${courseId}/materials`);
