...
export default function CourseExamEditor({
  ...
  async function handlePreview() {
    ...
    const result = await previewCourseExamSample(courseId);
      ...
      {activeTab === "random" && (
        <section className="mb-6 rounded-2xl border border-[#0F1B3D]/[0.08] bg-[#F7F8FA] p-5 sm:p-6">
          ...
          <div className="mt-4 flex gap-2">
            <button type="button" onClick={() => setBuildMode("preset")} className={`rounded-full px-4 py-2 text-[13px] font-bold ${buildMode === "preset" ? "bg-[#0F1B3D] text-white" : "bg-white text-[#0F1B3D]/60 border border-[#0F1B3D]/10"}`}>สุ่มตามระดับความยาก (Preset)</button>
            <button type="button" onClick={() => setBuildMode("custom")} className={`rounded-full px-4 py-2 text-[13px] font-bold ${buildMode === "custom" ? "bg-[#0F1B3D] text-white" : "bg-white text-[#0F1B3D]/60 border border-[#0F1B3D]/10"}`}>กำหนดเงื่อนไขรายบท (Custom)</button>
          </div>
          ...
          <label className="mt-4 block max-w-[200px]">
            ...
            <input type="number" min={1} value={totalQuestions} onChange={(e) => setTotalQuestions(Number(e.target.value))} className="w-full rounded-lg border border-[#0F1B3D]/10 bg-white px-3.5 py-2 text-sm outline-none focus:border-[#FF5A3C]" />
          </label>

          {buildMode === "preset" ? (
            <label className="mt-4 block max-w-md">
              <span className="mb-1.5 block text-[13px] font-semibold text-[#0F1B3D]/70">แม่แบบ</span>
              <select value={presetType} onChange={(e) => setPresetType(e.target.value as PresetType)} className="w-full rounded-lg border border-[#0F1B3D]/10 bg-white px-3.5 py-2 text-sm outline-none focus:border-[#FF5A3C]">
            ...
            <div className="mt-4 space-y-2.5">
              {customConstraints.map((constraint) => (
                <div key={constraint.key} className="flex flex-wrap items-center gap-2">
                  <select value={constraint.lessonId} onChange={(e) => updateConstraint(constraint.key, { lessonId: e.target.value })} className="rounded-lg border border-[#0F1B3D]/10 bg-white px-3 py-2 text-[13px]">
                    {lessons.map((lesson) => <option key={lesson.id} value={lesson.id}>{lesson.title}</option>)}
                  </select>
                  <select value={constraint.difficulty} onChange={(e) => updateConstraint(constraint.key, { difficulty: e.target.value as Difficulty })} className="rounded-lg border border-[#0F1B3D]/10 bg-white px-3 py-2 text-[13px]">
                    <option value="easy">ง่าย</option>
                    <option value="medium">ปานกลาง</option>
                    <option value="hard">ยาก</option>
                  </select>
                  <input type="number" min={1} value={constraint.count} onChange={(e) => updateConstraint(constraint.key, { count: Number(e.target.value) })} className="w-20 rounded-lg border border-[#0F1B3D]/10 bg-white px-3 py-2 text-[13px]" />
          ...
          {configError && <p role="alert" className="mt-4 rounded-xl bg-red-50 px-4 py-3 text-[13px] font-semibold text-red-700">{configError}</p>}
          ...
          <button type="button" disabled={configSaving} onClick={saveConfig} className="mt-4 rounded-full bg-[#0F1B3D] px-6 py-2.5 text-[13px] font-extrabold text-white disabled:opacity-60">
            {configSaving ? "กำลังบันทึก..." : "บันทึกกติกาสุ่มข้อสอบ"}
          </button>

          <button type="button" disabled={previewLoading} onClick={handlePreview} className="mt-2.5 ml-2.5 rounded-full border border-[#0F1B3D]/15 bg-white px-6 py-2.5 text-[13px] font-bold text-[#0F1B3D] disabled:opacity-60">
            {previewLoading ? "กำลังสุ่มตัวอย่าง..." : "ดูตัวอย่างชุดข้อสอบ"}
          </button>

          {previewError && (
            <p role="alert" className="mt-3 rounded-xl bg-red-50 px-4 py-3 text-[13px] font-semibold text-red-700">{previewError}</p>
          )}
